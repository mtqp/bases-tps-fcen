-- ACA VAN TODAS LAS CONSTRAINTS DE ETAPA

select "creando constraints de etapa" from DUAL;
...
FOR EACH ROW BEGIN
    ...
    IF 
    (   
        NEW.nombreEtapa <> 'FASE_GRUPOS'    AND 
        NEW.nombreEtapa <> '5TO_PUESTO'     AND 
        NEW.nombreEtapa <> '3ER_PUESTO'     AND 
        NEW.nombreEtapa <> 'SEMIFINAL'      AND 
        NEW.nombreEtapa <> 'FINAL'
    ) THEN
	    CALL `NombreEtapa es FASE_GRUPOS, 5TO/3ER_PUESTO, SEMIFINAL o FINAL`;
    END IF;
    
    CALL logOk('insert etapa', 'insert exitoso de etapa');
END$$
...
FOR EACH ROW BEGIN
    ...
    CALL logOk('update etapa', 'update exitoso de etapa');
END$$
